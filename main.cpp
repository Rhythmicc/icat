...
std::vector<std::pair<std::string, long long>> perf_records;
...
int main(int argc, char** argv) {
    ...
    if (perf_enabled) {
        std::cerr << "\n--- Performance Metrics ---\n";
        // Sort records by time to show total at bottom if it's there
        for (const auto& record : perf_records) {
            std::cerr << "[Perf] " << record.first << ": " << record.second << "ms\n";
        }
        std::cerr << "---------------------------\n";
    }
